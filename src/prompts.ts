...
export const DEFAULT_QUICK_PROMPTS: QuickPrompt[] = [
  ...
  },
  {
    ...
    icon: 'git-branch',
    enabled: true,
    prompt: '生成一个 Mermaid flowchart LR 代码块，用于交互式思维导图画布查看。要求：\n1. 只输出一个 Mermaid 代码块，不要输出解释文字\n2. 使用 `flowchart LR`，根节点放在最左侧，分支从左向右展开，整体形态接近树状思维导图\n3. 根节点为文章标题或核心主题\n4. 第一层为 4—7 个主要分支，每个分支再展开 2—4 个关键子节点\n5. 层级不超过 4 层，尽量让各分支层级均衡，不要让某一条链路过长\n6. 节点文字尽量短，不超过 12 个字\n7. 使用清晰的节点 id，例如 A、A1、A1a，不要在 id 中使用中文或特殊符号\n8. 节点显示文本用方括号包裹，例如 A[核心主题]\n9. 不要添加原文中没有的内容；如果原文信息不足，请使用“原文未提及”，不要编造'
  },
  {
    id: 'terms',
    label: '术语解释',
